...
static std::vector<std::pair<std::string, std::string>> validate_custom_gcode(const Print& print)
{
    ...
    if (ret.size() < MAX_TAGS_COUNT)
        check(_(L("Time lapse G-code")), config.time_lapse_gcode.value);
    if (ret.size() < MAX_TAGS_COUNT)
        ...
        if (auto it = custom_gcode_placeholders.find(name); it != custom_gcode_placeholders.end()) {
            ...
            for (const std::string& key : config_override->keys()) {
                ...
                if (std::find(placeholders.begin(), placeholders.end(), key) == placeholders.end()) {
                    auto& vector =
                        m_placeholder_error_messages[name + " - option not specified for custom gcode type (s_CustomGcodeSpecificOptions)"];
                                 ...
                                 const bool prime_extruder)
{
    ...
    if (!first_layer && !m_second_layer_things_done) {
        ...
        if (single_object_instance_idx == size_t(-1)) {
            ...
            gcode += ProcessLayer::emit_custom_gcode_per_print_z(*this, layer_tools.custom_gcode, m_writer.extruder()->id(),
        ...
        for (const LayerToPrint& layer_to_print : layers) {
            if (layer_to_print.support_layer != nullptr) {
                ...
                if (!support_layer.support_fills.entities.empty()) {
                    ...
                    if (!single_extruder && has_interface) {
                        ObjectByExtruder& obj_interface      = object_by_extruder(by_extruder, interface_extruder,
                                                                                  &layer_to_print - layers.data(), layers.size());
            ...
            if (layer_to_print.object_layer != nullptr) {
                ...
                // Traverse the slices in an increasing order of bounding box size, so that the islands inside another islands are tested
                // first, so we can just test a point inside ExPolygon::contour and we may skip testing the holes.
                ...
                for (size_t region_id = 0; region_id < layer.regions().size(); ++region_id) {
                    ...
                    for (const ObjectByExtruder::Island::Region::Type entity_type :
                        ...
                        for (const ExtrusionEntity* ee : (entity_type == ObjectByExtruder::Island::Region::INFILL) ?
                                                             layerm->fills.entities :
                                                             layerm->perimeters.entities) {
                            ...
                            if (is_anything_overridden) {
                                entity_overrides = const_cast<LayerTools&>(layer_tools)
                                                       .wiping_extrusions()
                                                       .get_extruder_overrides(extrusions, layer_to_print.original_object,
                                                                               correct_extruder_id,
        ...
        for (unsigned int extruder_id : layer_tools.extruders) {
            ...
            if (has_prime_tower) {
                ...
                instances_to_print = sort_print_object_instances(objects_by_extruder_it->second, layers, ordering,
                                                                 single_object_instance_idx);
            }
                ...
                for (InstanceToPrint& instance_to_print : instances_to_print) {
                    if (print.config().skirt_type == stPerObject && !instance_to_print.print_object.object_skirt().empty() &&
                        ...
                        if (first_layer)
                            m_skirt_done.clear();
                        const Point& offset = instance_to_print.print_object.instances()[instance_to_print.instance_id].shift;
                        gcode += generate_skirt(print, instance_to_print.print_object.object_skirt(), offset, layer_tools, layer,
                                                extruder_id);
                    ...
                    if (instance_to_print.object_by_extruder.support != nullptr) {
                        ...
                        if (is_overridden == (print_wipe_extrusions != 0))
                            gcode += this->extrude_support(
                                // support_extrusion_role is erSupportMaterial, erSupportTransition, erSupportMaterialInterface or erMixed
                                // for all extrusion paths.
    ...
    std::string GCode::change_layer(coordf_t print_z)
    {
        ...
        if (EXTRUDER_CONFIG(retract_when_changing_layer) && m_writer.will_move_z(z)) {
            ...
            gcode += this->retract(false, false,
                                   ZHopType(EXTRUDER_CONFIG(z_hop_types)) == ZHopType::zhtAuto ? LiftType::SpiralLift : lift_type);
        }
    ...
    std::string GCode::extrude_loop(ExtrusionLoop loop, std::string description, double speed, const ExtrusionEntitiesPtr& region_perimeters)
    {
        ...
        // as no neighbouring perimeter exists. If an internal perimeter exists, we should find 2 perimeters touching the de-retraction
        // point 1 - the currently printed external perimeter and 2 - the neighbouring internal perimeter.
        ...
        if (m_config.wipe_on_loops.value && paths.back().role() == erExternalPerimeter && m_layer != NULL &&
            m_config.wall_loops.value > 1 && paths.front().size() >= 2 && paths.back().polyline.points.size() >= 3) {
    ...
    static std::map<int, std::string> overhang_speed_key_map = {
        {1, "overhang_1_4_speed"}, {2, "overhang_2_4_speed"}, {3, "overhang_3_4_speed"}, {4, "overhang_4_4_speed"}, {5, "bridge_speed"},
    };
    ...
    double GCode::get_overhang_degree_corr_speed(float normal_speed, double path_degree)
    {
        ...
        double lower_speed_bound = lower_degree_bound == 0 ? normal_speed :
                                                             m_config.get_abs_value(overhang_speed_key_map[lower_degree_bound].c_str());
        double upper_speed_bound = upper_degree_bound == 0 ? normal_speed :
                                                             m_config.get_abs_value(overhang_speed_key_map[upper_degree_bound].c_str());
    ...
    std::string GCode::_extrude(const ExtrusionPath& path, std::string description, double speed)
    {
        ...
        if (m_config.enable_overhang_speed && !m_config.overhang_speed_classic && !this->on_first_layer() &&
            ...
            if (m_config.slowdown_for_curled_perimeters) {
                ...
                new_points = m_extrusion_quality_estimator.estimate_extrusion_quality(path, overhang_overlap_levels,
                                                                                      dynamic_overhang_speeds, ref_speed, speed,
        ...
        if (!variable_speed) {
            ...
            gcode += m_writer.set_speed(F, "", comment);
            {
                ...
                // BBS: use G1 if not enable arc fitting or has no arc fitting result or in spiral_mode mode or we are doing sloped
                // extrusion Attention: G2 and G3 is not supported in spiral_mode mode
                    ...
                    if (_mm3_per_mm > 0 && EXTRUDER_CONFIG(adaptive_pressure_advance) && EXTRUDER_CONFIG(enable_pressure_advance) &&
                        ...
                        if (last_set_speed > new_speed) { // Ramping down speed - use overhang logic where the minimum speed is used between
                            ...
                            sprintf(buf, ";%sT%u MM3MM:%g ACCEL:%u BR:%d RC:%d OV:%d\n",
                                    GCodeProcessor::reserved_tag(GCodeProcessor::ETags::PA_Change).c_str(), m_writer.extruder()->id(),
                                    _mm3_per_mm, acceleration_i, ((path.role() == erBridgeInfill) || (path.role() == erOverhangPerimeter)),
                                    1, // Force a dummy "role change" & "overhang perimeter" for the post processor, as, while technically
                                       // it is not a role change,
                                    // the properties of the extrusion in the overhang are different so it is technically similar to a role
                                    // change for the Adaptive PA post processor.
                                    1);
                        } else { // Ramping up speed - use baseline logic where max speed is used between current and upcoming extrusion
    ...
    std::string GCode::travel_to(const Point& point, ExtrusionRole role, std::string comment, double z /* = DBL_MAX*/)
    {
        ...
        if (needs_retraction && m_config.reduce_crossing_wall &&
        ...
        if (needs_retraction) {
            // ORCA: Fix scenario where wipe is disabled when avoid crossing perimeters was enabled even though a retraction move was
            // performed. This replicates the existing behaviour of always wiping when retracting
            /*if (m_config.reduce_crossing_wall && could_be_wipe_disabled)
                m_wipe.reset_path();*/

            Point last_post_before_retract = this->last_pos();
            gcode += this->retract(false, false, lift_type, role);
            // When "Wipe while retracting" is enabled, then extruder moves to another position, and travel from this position can cross
            // perimeters. Because of it, it is necessary to call avoid crossing perimeters again with new starting point after calling
            // retraction()
            // FIXME Lukas H.: Try to predict if this second calling of avoid crossing perimeters will be needed or not. It could save
            // computations.
    ...
    bool GCode::needs_retraction(const Polyline& travel, ExtrusionRole role, LiftType& lift_type)
    {
        ...
        auto is_through_overhang = [this](const Polyline& travel) {
            ...
            std::vector<size_t>      idx_of_object_sorted = m_curr_print->layers_sorted_for_object(z_range.first, z_range.second,
                                                                                                   layers_of_objects, boundingBox_for_objects,
                                                                                                   objects_instances_shift);
    ...
    std::string GCode::retract(bool toolchange, bool is_last_retraction, LiftType lift_type, ExtrusionRole role)
    {
        ...
        if (retract_lift_type == RetractLiftEnforceType::rletAllSurfaces) {
            ...
            can_lift = true;
        } else if (retract_lift_type == RetractLiftEnforceType::rletTopOnly ||
                   retract_lift_type == RetractLiftEnforceType::rletTopAndBottom) {
    ...
    std::string GCode::set_extruder(unsigned int extruder_id, double print_z, bool by_object)
    {
        ...
        if (!m_writer.multiple_extruders) {
            ...
            if (!filament_start_gcode.empty()) {
                ...
                config.set_key_value("long_retraction_when_cut",
                                     new ConfigOptionBool(m_config.long_retractions_when_cut.get_at(extruder_id)));
        ...
        if (m_writer.extruder() != nullptr || m_start_gcode_filament != -1) {
            ...
            // Orca: always calculate wipe volume and hence provide correct flush_length, so that MMU devices with cutter and purge bin
            // (e.g. ERCF_v2 with a filament cutter or Filametrix can take advantage of it)
    ...
    const std::vector<GCode::ObjectByExtruder::Island::Region>&
    GCode::ObjectByExtruder::Island::by_region_per_copy(std::vector<Region> & by_region_per_copy_cache, unsigned int copy,
                                                        unsigned int extruder, bool wiping_entities) const
    {
        ...
        for (const auto& reg : by_region) {
            ...
            for (int iter = 0; iter < 2; ++iter) {
                ...
                ExtrusionEntitiesPtr&                                        target_eec = (iter ? by_region_per_copy_cache.back().infills :
                                                                                                  by_region_per_copy_cache.back().perimeters);
    ...
    // This function takes the eec and appends its entities to either perimeters or infills of this Region (depending on the first
    // parameter) It also saves pointer to ExtruderPerCopy struct (for each entity), that holds information about which extruders should be
    // used for which copy.
    void GCode::ObjectByExtruder::Island::Region::append(const Type type, const ExtrusionEntityCollection* eec,
                                                         const WipingExtrusions::ExtruderPerCopy* copies_extruder)
    {
        // We are going to manipulate either perimeters or infills, exactly in the same way. Let's create pointers to the proper structure
